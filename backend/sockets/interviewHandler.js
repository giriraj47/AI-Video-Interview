...
// Grace period (ms) before destroying in-memory session after disconnect.
// If the same interviewId reconnects within this window, the timer is cancelled.
const SESSION_CLEANUP_DELAY_MS = 30_000;
const pendingCleanups = new Map(); // interviewId → timeoutId
...
export const setupInterviewSocket = (socket) => {
  ...
  socket.on("start_interview", async ({ interviewId }) => {
    try {
      ...
      // Cancel any pending cleanup from a previous disconnect for this session
      if (pendingCleanups.has(interviewId)) {
        clearTimeout(pendingCleanups.get(interviewId));
        pendingCleanups.delete(interviewId);
        console.log(
          `[Interview] Cancelled pending session cleanup for reconnected Interview ID: ${interviewId}`,
        );
      }

      console.log(
        `[Interview] Session start requested for Interview ID: ${interviewId}`,
      );
      ...
      if (!interview) {
        socket.emit("interview_error", {
          message: "Interview session not found in database",
        });
        return;
      }
      ...
      if (hasHistory) {
        console.log(
          `[Interview] Resuming active session for Interview ID: ${interviewId}`,
        );
        ...
        if (lastQuestion) {
          ...
          console.log(
            `[Interview] Resuming with repeated question: "${resumePrompt}"`,
          );

          const audioBuffer =
            await deepgramService.generateSpeech(resumePrompt);

          const questionCount =
            groqService.sessions[interviewId]?.questionCount || 1;
      ...
      console.log(
        `[Interview] Starting fresh session for Interview ID: ${interviewId}`,
      );

      // Generate the first greeting/question
      const aiDecision = await groqService.generateNextResponse(
        interviewId,
        null,
      );
      ...
      const audioBuffer = await deepgramService.generateSpeech(
        aiDecision.spoken_response,
      );
  ...
  socket.on("submit_audio", async (data) => {
    try {
      ...
      if (!interviewId) {
        socket.emit("interview_error", {
          message: "Interview session context lost",
        });
        return;
      }

      console.log(`[Interview] Received audio for session: ${interviewId}`);
      // Socket.io delivers binary payloads as Buffer directly — no base64 decoding needed
      const audioBuffer = Buffer.from(data.audioBuffer);

      // A. Speech to Text
      console.log(
        `[Interview] Submitting ${audioBuffer.byteLength} bytes of audio to Deepgram STT (Mimetype: ${data.mimetype})...`,
      );
      ...
      if (!transcript.trim()) {
        ...
        console.warn(
          `[Interview] Empty transcript received. Prompting user to repeat...`,
        );
        const fallbackAudio = await deepgramService.generateSpeech(
      ...
      console.log(
        `[Interview] Groq AI Decision received:`,
      ...
      const responseAudioBuffer = await deepgramService.generateSpeech(
        aiDecision.spoken_response,
      );
  ...
  socket.on("proctor_flag", async ({ type, description }) => {
    try {
      ...
      console.log(
        `[Proctor Flag] Saving flag to DB for ${interviewId}: ${type} - ${description}`,
      );
      await Interview.findByIdAndUpdate(interviewId, {
        $push: {
          flags: {
  ...
  socket.on("disconnect", () => {
    ...
    console.log(
      `[Socket] Client disconnected. Scheduling session cleanup in ${SESSION_CLEANUP_DELAY_MS / 1000}s for: ${interviewId}`,
    );

    // Don't stack multiple timers for the same session
    if (pendingCleanups.has(interviewId)) {
      clearTimeout(pendingCleanups.get(interviewId));
    }
    ...
    const timeoutId = setTimeout(() => {
      console.log(
        `[Socket] Grace period expired. Clearing session cache for: ${interviewId}`,
      );
      groqService.cleanupSession(interviewId);
      pendingCleanups.delete(interviewId);
    }, SESSION_CLEANUP_DELAY_MS);

    pendingCleanups.set(interviewId, timeoutId);
  });
};
